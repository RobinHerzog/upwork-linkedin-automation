...
class StrategyJWT {
  ...
  /*
  Define strategy for dashboard login ('users' collection, used username:password or access_code to generate JWT token)
  jwt_payload:
{
  "_id": "5c2e41abf732d02f882e2f45",
    ...
    passport.use('jwt-users', new JWTStrategy(jwtOpts, (jwt_payload, done) => {
      ...
      users_model.getOne(queryObj)
        .then(userDoc => {
          ...
          if (!userDoc) {
            const err = new Error('Bad token! User id, email and role given by payload are not correct.');
